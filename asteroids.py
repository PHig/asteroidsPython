...
# http://www.codeskulptor.org/#user16_FQPbCVORp6_11.py
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        ...
        while 1:
            ...
            if dist(random_pos,my_ship.get_position()) <= asteroid_info.get_radius() + 2 * my_ship.get_radius():
                continue
            else:
                rock_pos = random_pos
                break
        ...
        rock_vel = [random.random() * (.6 + increase) - (.3 + increase), random.random() * (.6 + increase) - (.3 + increase)]
...
def init():
    global rock_group,missile_group, lives, score, started,my_ship
